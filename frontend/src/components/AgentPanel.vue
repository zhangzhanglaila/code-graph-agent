...
<style scoped>
.agent-panel { padding: 16px; overflow-y: auto; height: 100%; background: var(--bg-card); border: 1px solid var(--border); border-radius: 8px; box-shadow: var(--shadow); }
...
.question-input {
  ...
  border-radius: 6px; background: var(--bg-elevated); color: var(--text); font-size: 14px;
}
.run-btn {
  padding: 8px 16px; border: none; border-radius: 6px;
  background: var(--accent, #2563eb); color: #fff; font-weight: 700; cursor: pointer;
}
.run-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.loading { padding: 20px; text-align: center; color: var(--text-dim); }
.error { padding: 12px; background: #fef2f2; border: 1px solid #fecaca; border-radius: 6px; color: var(--error); }
.section { margin-bottom: 16px; }
.section h4 { margin: 0 0 8px; font-size: 14px; color: var(--text-dim); }
.stat-row { display: flex; gap: 12px; flex-wrap: wrap; }
.stat { padding: 4px 8px; background: var(--bg-elevated); border: 1px solid var(--border); border-radius: 4px; font-size: 14px; font-weight: 600; }
.var-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(120px, 1fr)); gap: 8px; }
.var-card { padding: 8px; background: var(--bg-elevated); border: 1px solid var(--border); border-radius: 6px; }
.var-name { font-weight: 600; font-size: 14px; }
.var-values { font-size: 14px; color: var(--text-dim); }
.var-mutations { font-size: 14px; color: var(--warning); }
.reasoning-steps { display: flex; flex-direction: column; gap: 8px; }
.reason-step { padding: 10px; background: var(--bg-elevated); border: 1px solid var(--border); border-radius: 6px; border-left: 3px solid var(--accent, #2563eb); }
.step-header { display: flex; align-items: center; gap: 8px; margin-bottom: 6px; }
.step-num { width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; background: var(--accent, #2563eb); color: #fff; border-radius: 50%; font-size: 14px; font-weight: 700; }
.hypothesis { flex: 1; font-size: 14px; font-weight: 500; }
.confidence { font-size: 14px; padding: 2px 6px; border-radius: 10px; background: #e2e8f0; color: var(--text); font-weight: 700; }
.confidence.high { background: #dcfce7; color: #166534; }
.evidence-list { margin: 4px 0; }
.evidence { font-size: 14px; padding: 2px 6px; margin: 2px 0; color: var(--text-dim); }
.evidence.support { border-left: 2px solid #4caf50; }
.evidence.contradict { border-left: 2px solid #f44336; }
.conclusion { font-size: 14px; margin-top: 6px; font-style: italic; color: var(--text); }
.action-list { display: flex; gap: 8px; flex-wrap: wrap; }
.action-item { padding: 6px 10px; background: var(--bg-elevated); border: 1px solid var(--border); border-radius: 4px; display: flex; gap: 6px; align-items: center; }
.action-kind { font-weight: 500; font-size: 14px; }
.action-status { font-size: 14px; padding: 1px 4px; border-radius: 3px; background: #e2e8f0; font-weight: 700; }
.action-item.success .action-status { background: #dcfce7; color: #166534; }
.timing { font-size: 14px; color: var(--text-dim); margin-top: 8px; }
